...
#
#  - [1 ] reveal_grid        - subset 0
#  - [1 ] place_bombs        - subset 1
#  - [ 1] mark_cell          - subset 2
...
reveal_grid_loop_row:
        lw      $s0, ($sp)
        bge     $s0, N_ROWS, reveal_grid_loop_row_end #CHANGE?!??!?!?! to N_ROWS
...
place_bombs_loop_1:
        lw      $s0, ($sp)
        lw      $t0, total_bombs
        bge     $s0, $t0, place_bombs_loop_end        
        lw      $a0, 4($sp)
        lw      $a1, 8($sp)
        ...
        addi    $s0, $s0, 1
        sw      $s0, ($sp)     
        j       place_bombs_loop_1
place_bombs_loop_end:
...
mark_cell__body:
        ...
        add     $s0, $s0, $t0
        add     $s0, $s0, $a1      
        sw      $s0, 10($sp)     # $s0 = grid[row][col]
            # HELLLLLLLLLLOOOOOOOO?? BREAK HERE BEFORE BUT WHY??!
        lb      $t1, ($s0)
        and     $s1, $t1, IS_RVLD_MASK  # $s1 = grid[row][col] & IS_RVLD_MASK
        and     $s2, $t1, IS_MRKD_MASK  # $s2 = grid[row][col] & IS_MRKD_MASK
        sb      $s1, ($sp)
        sb      $s2, 1($sp)
        
        beq     $s1, 0, mark_cell__body_unmark_cell
        
        lw      $t0, debug_mode
        beq     $t0, FALSE, mark_cell__body_else
        add     $sp, $sp, 14               
        lw      $ra, 0($sp)
        addiu   $sp, $sp, 4
        jr      $ra        
mark_cell__body_else:
        la      $a0, mark_error
        li      $v0, 4
        syscall
        add     $sp, $sp, 14               
...
mark_cell__body_unmark_cell:        
        lb      $s2, 1($sp)
        beq     $s2, 0, mark_cell__body_unmark_cell_else 
        li      $t0, IS_MRKD_MASK     
        not     $t1, $t0
        lw      $s0, 10($sp)
        lb      $t2, ($s0)
        ...
        addi    $t3, $t3, 1
        sw      $t3, bomb_count
        j       mark_cell__body_unmark_cell_end
mark_cell__body_unmark_cell_else:
        li      $t0, IS_MRKD_MASK
        lw      $s0, 10($sp)
        lb      $t1, ($s0)
        or      $t1, $t1, $t0
        ...
        sub     $t2, $t2, 1
        sw      $t2, bomb_count
mark_cell__body_unmark_cell_end:
        add     $sp, $sp, 14                                
mark_cell__epilogue:
        lw      $ra, 0($sp)
        addiu   $sp, $sp, 4
...
reveal_cell__body:
        ...
        lb      $t0, ($s0)              #   if (grid[row][col] & IS_MRKD_MASK)
        and     $t1, $t0, IS_MRKD_MASK
        beq     $t1, 0, reveal_cell_not_marked        
        lw      $t0, debug_mode
        beq     $t0, FALSE, marked_else_not_debug
        add     $sp, $sp, 12
        j       reveal_cell__epilogue        
marked_else_not_debug:                  #print not debug
        la      $a0, reveal_error 
        li      $v0, 4
        syscall
        add     $sp, $sp, 12        
        j       reveal_cell__epilogue        
reveal_cell_not_marked:
        
        lw      $s0, ($sp)
        lb      $t0, ($s0)              #   if (grid[row][col] & IS_RVLD_MASK)
        andi    $t1, $t0, IS_RVLD_MASK
        beq     $t1, 0, reveal_cell_is_not_rvld        
        lw      $t2, debug_mode
        beq     $t2, FALSE, reveal_cell_alrdy_rvld
        add     $sp, $sp, 12                
        j       reveal_cell__epilogue
reveal_cell_alrdy_rvld:       
        la      $a0, already_revealed 
        li      $v0, 4
        syscall
        add     $sp, $sp, 12                
        j       reveal_cell__epilogue 
reveal_cell_is_not_rvld:        

        lb      $t0, ($sp)      #   // Trigger game over if the cell is a bomb.
        and     $t1, $t0, IS_BOMB_MASK
        beq     $t1, 0, reveal_cell_is_not_bomb
        li      $t2, LOSE
        sw      $t2, game_state        
reveal_cell_is_not_bomb:
        
        lw      $s0, ($sp)
        lb      $t0, ($s0)      #   // Reveal the cell.
        and     $t1, $t0, VALUE_MASK
        beq     $t1, 0, reveal_cell_is_value_mask
        lw      $a0, 4($sp)
        lw      $a1, 8($sp)
        jal     clear_surroundings
        j       reveal_cell_is_value_mask_end        
reveal_cell_is_value_mask:
        lw      $s0, ($sp)        
        lb      $t0, ($s0)
        ori     $t1, $t0, IS_RVLD_MASK
        sb      $t1, ($t3)
        lw      $t2, game_state
        bne     $t2, LOSE, reveal_cell_game_state 
        j       reveal_cell_is_value_mask_end        
reveal_cell_game_state:
        lw      $t0, cells_left
        sub     $t0, $t0, 1 
        sw      $t0, cells_left                
reveal_cell_is_value_mask_end:        
        
        lw      $t0, cells_left
        bne     $t0, 0, left_cells_lose
        li      $t0, 1
        sw      $t0, cells_left      
left_cells_lose:        
...
clear_surroundings__body:
        ...
        blt     $a0, 0, clear_surroundings_end0         #first if 
        bge     $a0, N_ROWS, clear_surroundings_end0
        blt     $a1, 0, clear_surroundings_end0
        bge     $a1, N_COLS, clear_surroundings_end0 
        j       clear_surroundings_end1
clear_surroundings_end0:                              
        addi    $sp, $sp, 12
        j       clear_surroundings__epilogue
clear_surroundings_end1:
                                                   
        lw      $s0, 8($sp)  
        lb      $s1, ($s0)     
                                
        andi    $t0, $s1, IS_RVLD_MASK  # second if
        bne     $t0, FALSE, clear_surroundings_is_rvld
        j       clear_surroundings_not_rvld
clear_surroundings_is_rvld:        
        add     $sp, $sp, 12        
        j       clear_surroundings__epilogue                
clear_surroundings_not_rvld:        
                
        lw      $s0, 8($sp)              #   // Reveal the cell.
        lb      $s1, ($s0)        
                
        ori     $s1, $s1, IS_RVLD_MASK
        sb      $s1, ($s0)
        lw      $t2, cells_left
        sub     $t2, $t2, 1
        sw      $t2, cells_left
        ...
        andi    $t0, $s1, VALUE_MASK
        bne     $t0, 0, clear_surroundings_is_numbered   # Stop revealing once a numbered cel
        j       clear_surroundings_not_numbered
clear_surroundings_is_numbered:        
        add     $sp, $sp, 12                
        j       clear_surroundings__epilogue                
clear_surroundings_not_numbered: 
        
        lw      $a0, ($sp)              #   clear_surroundings(row - 1, col - 1);
        lw      $a1, 4($sp)
        sub     $a0, $a0, 1
        sub     $a1, $a1, 1
        jal     clear_surroundings__prologue
        ...
        lw      $a0, ($sp)               #   clear_surroundings(row, col - 1);
        lw      $a1, 4($sp)
        sub     $a1, $a1, 1
        jal     clear_surroundings__prologue
        ...
        lw      $a0, 0($sp)             #   clear_surroundings(row + 1, col - 1);
        lw      $a1, 4($sp)
        addi    $a0, $a0, 1
        sub     $a1, $a1, 1
        jal     clear_surroundings__prologue         
        
        lw      $a0, 0($sp)             #   clear_surroundings(row + 1, col);
        lw      $a1, 4($sp)
        addi    $a0, $a0, 1
        jal     clear_surroundings__prologue
        ...
        lw      $a0, ($sp)                     # clear_surroundings(row - 1, col);
        lw      $a1, 4($sp) 
